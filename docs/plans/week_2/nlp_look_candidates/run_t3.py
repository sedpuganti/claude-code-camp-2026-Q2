...
ap.add_argument("--use-context", action="store_true",
                help="prepend the --context-fields as encoder context")
ap.add_argument("--context-fields", default="name,sector,exits",
                help="which room fields --use-context prepends. `sector` comes from the "
                     "world file and has NO runtime equivalent, so the SHIPPED model must "
                     "train with `name,exits` — see look_candidates_runtime.md 5.1.")
...
ap.add_argument("--save", default=None,
                help="directory to write the trained model + tokenizer to. Without this "
                     "the checkpoint is discarded at exit, which is how the entire first "
                     "bake-off ended up unshippable.")
args = ap.parse_args()

CTX_FIELDS = [f.strip() for f in args.context_fields.split(",") if f.strip()]
...
def encode(room):
    ...
    if args.use_context:
        # Context the model can attend to but must NOT be labelled - only
        # description tokens carry labels.
        parts = {"name":   room.get("name", ""),
                 "sector": (room.get("sector") or "").lower(),
                 "exits":  " ".join(room.get("exits", []) or [])}
        ctx = " ".join(parts[f] for f in CTX_FIELDS if f in parts)
...
if args.save:
    # The tokenizer is saved in its FAST form purely so build_model.rb can lift
    # the vocab out of tokenizer.json; nothing at runtime uses transformers.
    os.makedirs(args.save, exist_ok=True)
    model.save_pretrained(args.save)
    AutoTokenizer.from_pretrained(args.model).save_pretrained(args.save)
    json.dump({"base_model": args.model, "split": args.split, "seed": args.seed,
               "epochs": args.epochs, "maxlen": args.maxlen, "pos_weight": args.pos_weight,
               "context_fields": CTX_FIELDS if args.use_context else [],
               "tag": args.tag or args.split},
              open(os.path.join(args.save, "train_config.json"), "w"), indent=1)
