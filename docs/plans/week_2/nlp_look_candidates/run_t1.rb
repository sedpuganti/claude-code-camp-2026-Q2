...
#
# T1 — sparse logistic regression, pure Ruby, no dependencies.
#   ruby run_t1.rb
#
# The point of T1 is to test DATASET.md 3.1's claim directly: word identity alone
# is nearly useless (median purity 8.6%), so if CONTEXT features carry real signal
# this tier should clearly beat the T0c/T0d lexicons. If it doesn't, the task is
# either contextual in a way bag-of-features can't reach (-> T3) or dominated by
# authorial noise (-> README 7.4).
...
runner = lambda do |tr, te|
  ...
  tr.each do |r|
    g = r.gold.reduce(Set.new) { |a, s| a | s }
    featurize(r, lex, df).each { |(word, f)| X << f; Y << (g.include?(word) ? 1 : 0) }
  end
  warn "  [t1] train examples=#{X.size} pos=#{Y.sum} (#{(100.0 * Y.sum / X.size).round(2)}%)"
  w = fit(X, Y)
  warn "  [t1] weights=#{w.size}"
  $last_w = w
  te.map { |r| featurize(r, lex, df).map { |(word, f)| [word, predict(w, f)] } }
end

LC.report("T1 — logistic regression (pure Ruby, no deps)", SPLITS, runner)

puts "\n  top positive weights:"
$last_w.sort_by { |_k, v| -v }.first(14).each { |k, v| puts format("    %-22s %+.3f", k, v) }
puts "  top negative weights:"
$last_w.sort_by { |_k, v| v }.first(8).each { |k, v| puts format("    %-22s %+.3f", k, v) }
