...
module Boukensha
  class Client
    ...
    def call(max_output_tokens: 1024, tools: nil)
      ...
      # Net::HTTP enables VERIFY_PEER by default for HTTPS. Do not redundantly
      # assign verify_mode here: Ruby 4/OpenSSL 3.6 on native Windows can finish
      # the handshake and then lose the first response with
      # "SSL_read: unexpected eof" when it is assigned explicitly. Omitting
      # ca_file also lets OpenSSL honour SSL_CERT_FILE and platform defaults.
      ...
      loop do
        ...
        begin
          # Use an explicit session. Ruby 4/OpenSSL 3.6 on native Windows can
          # complete TLS but lose the response with "SSL_read: unexpected eof"
          # through Net::HTTP's one-shot #request auto-start path.
