module ManagerLog
  # Date-based lookup for manager log files. There is no per-id traversal
  # risk here the way there is for SessionLog::Store (§ path traversal test):
  ...
  class Store
    ...
    # nil when `date` is malformed or no file exists yet — "manager logging
    # is off" and "nothing happened today" are the same observable state
    # from here, and both render as an empty list rather than an error
    # (spec §4.3: off by default).
    def path_for(date)
