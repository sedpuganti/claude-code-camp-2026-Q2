...
#
# Native Windows defaults to the plain REPL because Bubble Tea's alternate
# screen is not reliable in Git Bash. --tui explicitly overrides that choice;
# --no-tui selects plain mode on every platform.
#
# Examples:
...
module BoukenshaLoader
  ...
  def self.load_and_start_repl
    ...
    # Bubble Tea's native Windows alternate-screen implementation can open a
    # blank screen under Git Bash. Default to the reliable plain REPL there;
    # WSL reports a Linux Ruby platform and therefore retains the TUI default.
    no_tui = ARGV.delete("--no-tui")
    force_tui = ARGV.delete("--tui")
    native_windows = Gem.win_platform?
    use_tui = force_tui || (!no_tui && !native_windows)

    if native_windows && !no_tui && !force_tui
      warn "[boukensha] native Windows detected; using plain REPL. " \
           "Run under WSL for the TUI, or pass --tui to force it."
    end
