...
module Boukensha
  module Mcp
    ...
    class Client
      ...
      # Windows PATH entries installed by Node and Ruby often contain both an
      # extensionless POSIX shim and a native .cmd/.bat wrapper. Process.spawn
      # may select the extensionless file first (notably for `npx`), leaving
      # the MCP handshake waiting forever. Prefer PATHEXT executables just as
      # cmd.exe does; Open3 can launch the resulting full wrapper path.
      def resolve_command(command)
        ...
        ENV.fetch("PATH", "").split(File::PATH_SEPARATOR).each do |directory|
          ...
          extensions.each do |extension|
            candidate = File.join(directory, "#{command}#{extension.downcase}")
            return candidate if File.file?(candidate)

            candidate = File.join(directory, "#{command}#{extension.upcase}")
            return candidate if File.file?(candidate)
          end
        end
        command
      ...
      def startup_timeout!
        # Close the stdio transport first so wrapper descendants (for example
        # npx.cmd -> node.exe) also observe EOF and exit. Killing only the
        # wrapper can leave its child holding stderr open forever.
        @stdin.close rescue nil
        @stdout.close rescue nil
        @stderr.close rescue nil
