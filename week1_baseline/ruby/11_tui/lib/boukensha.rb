...
module Boukensha
  ...
  def self.repl(
    ...
    if tui
      begin
        require_relative "boukensha/tui" unless defined?(Tui)
      rescue LoadError => e
        warn "[boukensha] TUI unavailable (#{e.message}); using plain REPL. " \
             "On native Windows use --no-tui, or run the TUI under WSL."
      end
    end
