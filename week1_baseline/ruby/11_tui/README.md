...
Boukensha now ships a full terminal UI (TUI) built on the Charm libraries
`bubbletea`, `lipgloss`, and `bubbles`. It deliberately does not depend on the
`charm` meta-gem because that also installs the unused `ntcharts` native
extension, which does not ship a Windows Go archive. The plain REPL is still
there and can be selected with `tui: false`.

The TUI libraries are optional at gem-install time. Their upstream
Ruby-platform packages do not contain the Go archives required by native
Windows, so native Windows automatically falls back to the plain REPL; use
`boukensha --no-tui` explicitly or run the full TUI under WSL/Linux. This keeps
installation usable without disabling TLS checks or requiring a Go toolchain.
...
itself needs only `bubbletea`, `lipgloss`, and `bubbles` for the TUI.
