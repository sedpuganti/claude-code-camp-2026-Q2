...
class Tui(App):
    ...
    def _tick(self) -> None:
        # A final interval callback may already be queued while Textual is
        # dismantling the default screen. In that narrow window the app is
        # alive but its widgets are no longer queryable.
        try:
            self._drain_events()
            if self._live["active"] and self._live["start_time"] is not None:
                self._live["spinner_idx"] = (
                    self._live["spinner_idx"] + 1
